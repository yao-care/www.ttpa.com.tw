...
調整活動、課程、成果或特殊公告時，必須一併檢查 `/news/`。頁面會依內容資料自動彙整：

- `status` 顯示為報名中、開放報名、招生中或招募中的活動與課程。
- 最近一年內的活動成果；其中近半年成果為近期動態，超過半年後仍保留至滿一年。
- 最近半年內的特殊事件公告（例如延期、取消、場地或時間異動）。已結束的一般活動公告不重複顯示。

活動或課程結束時，應更新原項目的 `status`；有成果資料時另新增或更新 `src/content/results/`，不需要手動修改最新消息列表。
